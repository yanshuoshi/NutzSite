 ...
 */
@IocBean
@At("/api/api/checkpoint")
public class ApiCheckpointController {
    ...
    public Object list(@Param("pageNum")int pageNum,
        ...
        List<Checkpoint> list = checkpointService.query(cnd,pageNum,pageSize);
    ...
    public Object get(@Param("id") String id) {
        try {
            return Result.success("system.success",checkpointService.fetch(id));
        } catch (Exception e) {
    ...
    @Ok("json")
    @AccessToken
    public Object addDo(@Param("..") Checkpoint checkpoint, Errors es, HttpServletRequest req) {
        try {
            if (es.hasError()) {
    ...
    @Ok("json")
    @AccessToken
    public Object editDo(@Param("..") Checkpoint checkpoint, Errors es, HttpServletRequest req) {
        try {
            if (es.hasError()) {
                return Result.error(es);
            }
            if(Lang.isNotEmpty(checkpoint)){
				checkpoint.setUpdateBy(JWTUtil.getId());
